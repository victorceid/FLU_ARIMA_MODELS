...
ARIMA_ES27<-function(grouped_data=NULL, US_STATE=3, auto=TRUE, my_n_ahead=1){
  ...
  for(i in 1:NROW(all_data) ){
    my_preds_list[[i]]%<-% PredictByIteration2(all_data, my_n_ahead=my_n_ahead, look_back_amount = 104, order_params=my_order_params,auto_seasonal = FALSE, auto=auto, my_seasonal=list(order = c(1,0,1), period = 0) ) %packages% "forecast" 
  }
  ...
  my_errors<- GetErrors_Each_State(US_STATE, my_data = grouped_data, prediction = all_preds) 
  my_errors<-data.frame(my_errors[[1]])
